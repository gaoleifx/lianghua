...
class EntrypointTests(unittest.TestCase):
    def test_entrypoints_share_runtime_and_have_no_secret(self):
        with open(os.path.join(ROOT, "08196542-1ba7-11f1-8c86-107c6107318f", "automation_config.json"), encoding="utf-8") as handle:
            cfg = json.load(handle)
        paths = [os.path.join(cfg["sim_working_directory"], cfg["sim_entry"]),
                 os.path.join(cfg["live_working_directory"], cfg["live_entry"])]
        for path in paths:
    ...
    def test_live_entry_uses_live_account_and_separate_state(self):
        with open(os.path.join(ROOT, "08196542-1ba7-11f1-8c86-107c6107318f", "automation_config.json"), encoding="utf-8") as handle:
            cfg = json.load(handle)
        path = os.path.join(cfg["live_working_directory"], cfg["live_entry"])
        with open(path, encoding="utf-8") as handle: text = handle.read()
    ...
    def test_simulation_entry_uses_simulation_account_and_mode(self):
        with open(os.path.join(ROOT, "08196542-1ba7-11f1-8c86-107c6107318f", "automation_config.json"), encoding="utf-8") as handle:
            cfg = json.load(handle)
        path = os.path.join(cfg["sim_working_directory"], cfg["sim_entry"])
        with open(path, encoding="utf-8") as handle: text = handle.read()
    ...
    def test_manual_live_authorization_enables_live_entries(self):
        ...
        self.assertEqual("manual_live_authorization_20260829", config["deployment"]["validation_status"])
